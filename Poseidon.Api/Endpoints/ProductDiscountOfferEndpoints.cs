...
{
    app.MapGet("api/productdiscountoffer/query",
    ...
    async (    [FromServices] IMediator _mediator
             , [AsParameters] QueryCommand<ProductDiscountOfferDto> command
    ) =>
    {
        return Results.Ok(await _mediator.Send(command));
    }).RequireAuthorization();
    
    app.MapPost("api/productdiscountoffer/upsert", 
    ...
    app.MapDelete("api/productdiscountoffer/delete", 
    [ProducesResponseType(200, Type = (typeof(int)))]
    [ProducesResponseType(404, Type = (typeof(int)))]
    [ProducesResponseType(500, Type = (typeof(ErrorResponse)))]
    async ( [FromServices] IMediator          _mediator
          , [AsParameters] DeleteCommand<ProductDiscountOffer> command) =>
    {
       var (id, status) = await _mediator.Send(command);
